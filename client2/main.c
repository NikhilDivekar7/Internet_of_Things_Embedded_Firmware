...
void TimerInitFunc(int duty_cycle)
{
	...
	TIMER0->ROUTELOC0 |= TIMER_ROUTELOC0_CC2LOC_LOC26;
...
void main(void)
{
    ...
    car_count = ps_streetlight.count_of_vehicles;

    char buff[16];
	snprintf(buff, "Car count : %d", car_count);
    ...
    switch (BGLIB_MSG_ID(evt->header)) {
      ...
      case gecko_evt_le_connection_closed_id:
       ...
       case gecko_evt_sm_confirm_passkey_id:	//MITM protection
    	   ...
    	   //acknowledging MITM passkey with push buttons
       ...
       case gecko_evt_gatt_procedure_completed_id:
    	   ...
    	   //state machine to get the 2 services and their characteristics
           	switch(_main_state)
           	{
           	case FIND_CHAR2:
			if (_char_handle1 > 0)
...
//          			GRAPHICS_Update();
          			...
          			if(*evt->data.evt_gatt_characteristic_value.value.data < 20)	//night
					{
						int i;
					...
					else if (*evt->data.evt_gatt_characteristic_value.value.data > 20)	//day
					{
...
//          		          			snprintf(buf,40,"Client: %d\n", final_value);
			...
			if(light_flag == 1)
			{
				...
				//total time elapsed
				GRAPHICS_Clear();
				GRAPHICS_AppendString("Total count: %d \n", night_total_count);
				GRAPHICS_Update();

				//time for which street light was on at low intensity
				GRAPHICS_Clear();
				GRAPHICS_AppendString("Half count: %d \n", count_halfon);
				GRAPHICS_Update();

				//time for which street light was on at full intensity
				GRAPHICS_Clear();
				GRAPHICS_AppendString("Full on count: %d \n", count_on);
				GRAPHICS_Update();

				//consumption
				energy_consumption = (count_halfon/2) + (count_on);

				GRAPHICS_Clear();
				GRAPHICS_AppendString("Energy consumption: %d \n", energy_consumption);
				GRAPHICS_Update();

				energy_saving = night_total_count - energy_consumption;
				//total savings
				GRAPHICS_Clear();
				GRAPHICS_AppendString("Energy saving: %d \n", energy_saving);
				GRAPHICS_Update();

						//distance when obstacle detected
						...
						}
						{
							if(currentval == 0 && flag == 1)
							{
								...
								ps_streetlight.count_of_vehicles = car_count;
								persistent_state_store();
								char buf1[16] = {0};
